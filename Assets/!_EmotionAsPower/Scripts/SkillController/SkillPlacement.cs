...
public class SkillPlacement : MonoBehaviour
{
    ...
    [SerializeField] private ObjectSkillDatabaseSO database;
    ...
    public void StartPlacement(int objectIndex)
    {
        ...
        selectedObjectIndex = database.skills.FindIndex(b => b.skillSO.skillID == objectIndex);
    ...
    {
        Debug.Log("Attempting to place structure: " + database.skills[selectedObjectIndex].name);
        ...
        GameObject gameObject = Instantiate(database.skills[selectedObjectIndex].gameObject, gridPosition, Quaternion.identity);
        gameObject.transform.position = grid.CellToWorld(gridPosition);
        Debug.Log("Placed structure: " + database.skills[selectedObjectIndex].name + " at position: " + gridPosition);
